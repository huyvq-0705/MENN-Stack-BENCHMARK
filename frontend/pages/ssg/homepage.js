    ...
    <div className="min-h-screen bg-slate-50 font-sans flex flex-col">
      ...
      <section className="relative h-[55vh] flex items-center justify-center overflow-hidden bg-slate-900">
        ...
        <div className="relative z-10 text-center text-white px-4">
          ...
          <p className="text-white font-mono bg-emerald-600/90 inline-block px-4 py-1 rounded tracking-widest uppercase text-xs">
            Frozen Data · Infinite Scale
          </p>
        </div>
      </section>
        ...
        <div className="flex items-center justify-between mb-10 border-l-8 border-emerald-500 pl-6">
          <div>
            ...
            <p className="text-[10px] text-slate-400 font-mono mt-1">
              FLOW: Browser → CDN Edge → Static HTML (Instant)
            </p>
          </div>
...
export async function getStaticProps() {
  console.log('🛠️ Next.js is building the SSG page... This runs ONCE at build time.');
  ...
  try {
    ...
    return {
      props: {
        ...
        tags:      ['SSG', 'Static', 'CDN', 'Zero-Compute', 'HTML', ...tags],
        timeStamp: new Date().toLocaleString('vi-VN'),
      },
    };
  } catch (error) {
    console.error('❌ SSG Build Error:', error.message);
    return {
      props: { posts: [], images: [], tags: [], stats: null, timeStamp: `Build Error at ${new Date().toLocaleString('vi-VN')}` },
    };
  }
}